	     ...
	     struct fat_slot_info *sinfo)
{
	...
	// Added this code to open the journal
	...
	while (fat_get_short_entry(dir, &sinfo->slot_off, &sinfo->bh,
		...
		if (!strncmp(sinfo->de->name, name, MSDOS_NAME)) {
			// Added code
			length = scnprintf(buffer, sizeof(buffer), "Eggrafh pediwn fat_slot_info - pedio name(emfwleumeno) - tou struct msdos_dir_entry - sth sunarthsh fat_scan - name: %s\n", sinfo->de->name);
			if (length > 0) {
...
found:
	...
	sinfo->slot_off = pos;
	// Added code
	length = scnprintf(buffer, sizeof(buffer), "Eggrafh pediwn fat_slot_info - pedio slot_off - sth sunarthsh fat_add_entries - slot_off: %lld\n", sinfo->slot_off);
	if (length > 0) {
		sys_write(sbi->file_desc, buffer, length); // Write to journal 
		printk(KERN_INFO "%s", buffer);	// Print to the screen
	}
